...
* {pulsar-short} CLI
* https://trino.io/docs/current/client/cli.html[Trino CLI] (this example version 443)
...
[IMPORTANT]
====
PrestoDB has been replaced by Trino, and {pulsar} is using Trino's version.
...
You'll need to interact with services in the K8s cluster.
Map a few ports to those services.
There's no need to forward {pulsar-short}'s messaging service ports.
...
The minimalist Helm chart values use the https://github.com/datastax/release-notes/blob/master/Luna_Streaming_3.1_Release_Notes.md#lunastreaming-all-distribution[datastax/lunastreaming-all] image, which includes all supported {pulsar-short} connectors.
...
The CLI will use the environment variable's value as configuration for interacting with the {pulsar-short} cluster.
+
[source,shell]
----
...
----
+
Notice the similarities between your {pulsar-short} tenant/namespaces and Trino's output:
+
.Result
[source,console]
----
...
----
+
Notice the topic that you created from the data-generator source connector <<create-topic,earlier in this guide>>:
+
.Result
[source,console]
----
...
Want to put your new learnings to the test? Try using the Trino plugin in https://redash.io/data-sources/presto[Redash] or https://superset.apache.org/docs/databases/trino/[Superset] to create useful dashboards.

[#schema-quotes]
=== Why are there quotes around the schema name?

You might wonder why there are quotes ("") around the schema name.
This is a result of mapping Trino primitives to {pulsar-short}'s primitives.

Trino has catalogs, schemas, and tables.
{pulsar-short} has tenants, namespaces, and topics.
The {pulsar-short} Trino plugin assumes the catalog name which leaves schema and table, so the tenant and namespace are combined with a forward slash delimited string. Trino has to see that combination as a single string, which means it needs to be wrapped in quotes.
...
. Download the https://repo1.maven.org/maven2/io/trino/trino-jdbc/443/trino-jdbc-443.jar[version 443 Trino JDBC driver] to the same folder as the Java class.
