...
. Download the heartbeat binary `.gz` file matching your OS and process architecture from the {pulsar-heartbeat-repo}/releases[releases page].
...
The {pulsar-short} Heartbeat configuration is defined by a `.yaml` file. A yaml template for Heartbeat is available at {pulsar-heartbeat-repo}/blob/master/config/runtime-template.yml[]. In this file, the environmental variable `PULSAR_OPS_MONITOR_CFG` tells the application where to source the file.
