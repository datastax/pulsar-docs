...
This document explains installation of {product} for Bare Metal/VM deployments with a {pulsar-short} tarball.
...
* *Tiered Storage:* Offload historical messages to more cost effective object storage such as AWS S3, Azure Blob, Google Cloud Storage, and HDFS.
...
* {bookkeeper-reg} should use one volume device for the journal, and one volume device for the ledgers.
...
* {company} recommends a separate data disk volume for {zookeeper-short}.
...
|Contains the core {pulsar-short} modules: {zookeeper-short}, broker, {bookkeeper-short}, and function worker
...
. Run `ls -al` to view your {product} files:
+
[source,console]
----
...
Download the latest version from the {pulsar-heartbeat-repo}/releases/[{company} GitHub repo] and follow the instructions xref:components:heartbeat-vm.adoc[here].

== Next steps

* For initializing {pulsar-short} components like {bookkeeper-short} and {zookeeper-short}, see the https://pulsar.apache.org/docs/deploy-bare-metal[{pulsar-short} documentation].
