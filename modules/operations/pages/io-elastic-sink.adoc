...
The https://www.elastic.co/elasticsearch/[Elasticsearch] sink connector reads messages from {pulsar-short} topics and persists messages to indexes.
...
. Start a {pulsar-short} service locally in standalone mode.
+
[source,bash]
----   
...
+
. Start the {pulsar-short} Elasticsearch connector in local run mode using the JSON or YAML configuration file. 
+
[tabs]
====
JSON::
