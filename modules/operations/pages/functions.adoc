...
Add functions using the {pulsar-short} Admin CLI. Create a new Python function to consume a message from one topic, add an exclamation point, and publish the results to another topic.
...
. Deploy `function.py` to your {pulsar-short} cluster using the {pulsar-short} Admin CLI:
+
[source,bash]
----
...
To test a function with the {pulsar-short} CLI, send a test value with {pulsar-short} CLI's `trigger`.
...
----
+
The trigger sends the string `Hello world` to your exclamation function.
Your function should output `Hello world!` to your consumed output.
...
For more information, see the https://pulsar.apache.org/docs/en/functions-develop/[{pulsar-short} documentation on developing functions].